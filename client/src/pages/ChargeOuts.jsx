...
export default function ChargeOuts() {
  ...
  useEffect(() => {
    if (activeTab === 'glSwaps') loadSwaps();
  }, [activeTab, filterMonth, filterYear]);

  // Also load departments for swap modal (reuse existing departments state loaded in chargeOuts tab)
  useEffect(() => {
  ...
  const handleSwapSubmit = async (e) => {
    ...
    setSwapError('');
    try {
      await api.glSwaps.create(swapForm);
  ...
  const handleSwapDelete = async (id) => {
    try {
      await api.glSwaps.delete(id);
      setSwapDeleteConfirm(null);
      loadSwaps();
    } catch (err) {
      alert(err.message);
    }
  ...
  // Find item name for swap modal display
  const selectedSwapPO = purchaseOrders.find(p => p.id === parseInt(swapForm.purchase_order_id));
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex border-b border-gray-200 mb-5">
        <button
          onClick={() => setActiveTab('chargeOuts')}
      ...
      <div className="flex items-center justify-between mb-5 gap-4">
        ...
        {activeTab === 'glSwaps' && glSwapList.length > 0 && (
          <div className="flex items-center gap-4">
            ...
            <a
              href={api.glSwaps.csvUrl(filterMonth, filterYear)}
                ...
                <tbody className="divide-y divide-gray-50">
                  {glSwapList.map(gs => (
                    <tr key={gs.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-5 py-3.5">
                        {gs.po_number
                          ? <span className="font-mono text-xs bg-brand-50 text-brand-700 px-2 py-0.5 rounded">{gs.po_number}</span>
                          : <span className="text-gray-400">—</span>}
                      </td>
                      <td className="px-5 py-3.5 font-medium text-gray-900">{gs.item_name}</td>
                      <td className="px-5 py-3.5">
                        <div className="text-gray-700">{gs.from_department_name}</div>
                        <span className="font-mono text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{gs.from_gl_number}</span>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="text-gray-700">{gs.to_department_name}</div>
                        <span className="font-mono text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{gs.to_gl_number}</span>
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <button
                          onClick={() => setSwapDeleteConfirm(gs)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors"
                        >
                          <Trash2 size={15} />
                        </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={form.charged_by} readOnly
                />
              </div>
              <div>
          ...
          <form onSubmit={handleSwapSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  {purchaseOrders.map(po => (
                    <option key={po.id} value={po.id}>
                      {po.po_number || '(no PO #)'} — {po.item_name} — {po.received_at}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input
                  className="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-500 cursor-not-allowed"
                  value={selectedSwapItem?.name || (swapForm.purchase_order_id ? 'Loading...' : '— select a PO first —')}
                  readOnly
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  className="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-500 cursor-not-allowed"
                  value={swapForm.swapped_by} readOnly
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Notes</label>
      ...
      {/* Delete Charge-Out confirmation */}
      {deleteConfirm && (
      ...
      {/* Delete GL Swap confirmation */}
      {swapDeleteConfirm && (
        <Modal title="Delete GL Swap" onClose={() => setSwapDeleteConfirm(null)} size="sm">
          <p className="text-gray-600 text-sm">
            Delete this GL Swap of <strong>{swapDeleteConfirm.item_name}</strong> from <strong>{swapDeleteConfirm.from_department_name}</strong> to <strong>{swapDeleteConfirm.to_department_name}</strong> on <strong>{fmtDate(swapDeleteConfirm.swapped_at)}</strong>?
          </p>
          <div className="flex justify-end gap-3 mt-5">
            <button onClick={() => setSwapDeleteConfirm(null)}
              className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
              Cancel
            </button>
            <button onClick={() => handleSwapDelete(swapDeleteConfirm.id)}
              className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors">
              Delete
            </button>
          </div>
